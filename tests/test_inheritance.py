...
from jinja2 import DictLoader, Environment, TemplateRuntimeError, TemplateSyntaxError
...
class TestInheritance:
    ...
    def test_super_in_scoped_block(self, env):
        ...
        t = env.from_string('{% extends "default.html" %}{% block item %}'
    ...
    def test_level1_required(self, env):
        env = Environment(loader=DictLoader({
            "default":
            "{% block x required %}{# comment #}\n {% endblock %}",
            "level1":
            "{% extends 'default' %}{% block x %}[1]{% endblock %}",
        }))
    ...
    def test_level2_required(self, env):
        env = Environment(loader=DictLoader({
            ...
            "{% extends 'default' %}{% block x %}[1]{% endblock %}",
            "level2":
            "{% extends 'default' %}{% block x %}[2]{% endblock %}",
        }))
    ...
    def test_level3_required(self, env):
        ...
        with pytest.raises(TemplateRuntimeError,
                           match="Required block 'x' not found"):
            assert t1.render()
    ...
    def test_invalid_required(self, env):
        env = Environment(loader=DictLoader({
            ...
            "{% endblock %}  {% endblock %}",
            "default2":
            "{% block x required %}{% if true %}"
            "{% endif %}  {% endblock %}",
            "level1":
            "{% if default %}{% extends default %}"
    ...
    def test_required_with_scope(self, env):
        env = Environment(loader=DictLoader({
            ...
            "{{ item }}{% endblock %}",
            "default2":
            "{% for item in seq %}[{% block item required scoped %}"
            "{% endblock %}]{% endfor %}",
            "child2":
            "{% extends 'default2' %}{% block item %}"
            "{{ item }}{% endblock %}",
        }))
