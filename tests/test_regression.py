...
class TestBug:
    def test_keyword_folding(self, env):
        ...
        assert (env.from_string("{{ 'test'|testing(some='stuff') }}").render() ==
                "teststuff")
    ...
    def test_urlize_filter_escaping(self, env):
        ...
        assert (tmpl.render() ==
                '<a href="http://www.example.org/&lt;foo" rel="noopener">'
...
# endif
    ...
    def test_empty_if_condition_fails(self, env):
        ...
        pytest.raises(TemplateSyntaxError, Template,
                      "{% if foo %}...{% elif %}...{% endif %}")
        pytest.raises(TemplateSyntaxError, Template,
                      "{% for x in %}..{% endfor %}")
    ...
    def test_macro_scoping(self, env):
        ...
        """)
        assert list(map(int,
                        tmpl.render().split())) == [3, 2, 1, 5, 4, 3, 7, 6, 5]
    ...
    def test_variable_reuse(self, env):
        ...
        t = env.from_string(
            "{% for x in x.y %}{{ loop.index0 }}|{{ x }}{% endfor %}")
    ...
    def test_subproperty_if(self, env):
        ...
        assert (t.render(object1={"subproperty1": "value"},
                         object2={"subproperty2": "value"}) == "42")
